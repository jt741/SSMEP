...
class KringlebotnMiddleEar:
    ''' converted from cgs units to mks'''
    def __init__(self):
        #Middle Ear Cavities
        self.L_a = Inductor(1*10**-3*10**5)
        self.C_a = Capacitor(3.9*10**-6*10**-5)
        self.R_a = Resistor(60*10**5)

        self.C_t = Capacitor(0.4*10**-6*10**-5)

        #eardrum inertance
        self.L_d = Inductor(7.5*10**-3*10**5)

        #eardrum suspension
        self.L_s = Inductor(66*10**-3*10**5)
        self.C_s = Capacitor(0.3*10**-6*10**-5)
        self.R_s = Resistor(20*10**5)

        self.C_r = Capacitor(1.3*10**-6*10**-5)
        self.R_r = Resistor(120*10**5)
        

        #coupling eardrum-manubrium
        self.C_m = Capacitor(0.38*10**-6*10**-5)
        self.R_m = Resistor(120*10**5)
        

        #malleus incus ligaments tensor tympani
        self.L_o = Inductor(22*10**-3*10**5)
        # self.C_o = Capacitor(10000000) # infinity
        self.R_o = Resistor(20*10**5)
        

        # coupling incus-stapes
        self.C_i = Capacitor(0.3*10**-6*10**-5)
        self.R_i = Resistor(6000*10**5)
    

        #cochlea stapes stapedius tendon windows
        self.L_c = Inductor(46*10**-3*10**5)
        self.C_c = Capacitor(0.56*10**-6*10**-5)
        self.R_c = Resistor(330*10**5)
...
class KringlebotnMiddleEar_fixed_stapes:
    ''' converted from cgs units to mks'''
    def __init__(self):
        #Middle Ear Cavities
        self.L_a = Inductor(1*10**-3*10**5)
