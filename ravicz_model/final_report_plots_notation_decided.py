...
middle_ear_effusion_state_norm_u = {
    "0" : end_to_end_get_ar_response_norm_velocity(normal_middle_ear),
    "25" : end_to_end_get_ar_response_norm_velocity(effused_middle_ear_25),
    #"40" : end_to_end_get_ar_response(effused_middle_ear_40),
    "50" : end_to_end_get_ar_response_norm_velocity(effused_middle_ear_50),
    #"70" : end_to_end_get_ar_response(effused_middle_ear_70),
    "100" : end_to_end_get_ar_response_norm_velocity(effused_middle_ear_100),
}
...
def plot_ravicz_changing_effusion_for_latex(effused_dict, reflectivity=True):
    ...
    if reflectivity:
        ...
        ax.set(
            #title="Modelled acoustic reflectometry response with changing effusion level", 
            xlabel="Frequency (Hz)", 
            ylabel=r"$|Z^s_{\mathrm{EC}}|\thinspace(\mathrm{kg} \mathrm{m}^{-2} \mathrm{s}^{-1})$",
            )
    ...
    if reflectivity:
        plt.savefig("ravicz_model/final_report_plots_notation/different_effusions_latex_reflectivity.pdf")
    else:
        plt.savefig("ravicz_model/final_report_plots_notation/different_effusions_latex.pdf")
    plt.show()
...
def compare_ravicz_with_real_for_latex(ravicz_dict, measured_data, reflectivity=True):
    ...
    if reflectivity:
        plt.savefig("ravicz_model/final_report_plots_notation/comparison_latex_refl.pdf")
    else:
        plt.savefig("ravicz_model/final_report_plots_notation/comparison_latex.pdf")
...
def compare_reflectivity_to_norm_u(reflectivity_and_norm_u_dict):
    ...
    plt.savefig(f"ravicz_model/final_report_plots_notation/reflectivity_v_impedance.pdf")
    plt.show()
...
def compare_ravicz_impedances_for_latex(ear_dict:dict, ear_canal=False):
    ...
    plt.savefig(f"ravicz_model/final_report_plots_notation/{impedance_type}_impedance_bode_ravicz_latex.pdf")
    plt.show()
...
def spectral_angle_v_tm_coverage_for_latex(effusion_ar_resp:dict):
    ...
    plt.savefig("ravicz_model/final_report_plots_notation/spectral_angle_v_tm_cover.pdf")            
    plt.show()
...
tm_and_effusion_modified_range_for_spectral_angle_norm_u_manual = {
    "0": end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(), start_f=2500, stop_f=3500, num=5000),
    "25": end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(C_MEC=6.5*10**-12, M_TOC=0.4*10**3, R_TOC=103*10**6), start_f=2500, stop_f=3500, num=5000),
    "40": end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(C_MEC=6.1*10**-12, M_TOC=1.5*10**3, R_TOC=110*10**6),start_f=2500, stop_f=3500, num=5000),
    "50": end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(C_MEC=5.4*10**-12, M_TOC=25*10**3, R_TOC=95*10**6), start_f=2500, stop_f=3500, num=5000),
    "70": end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(C_MEC=5.0*10**-12, M_TOC=32*10**3, R_TOC=130*10**6),start_f=2500, stop_f=3500, num=5000),
    "100": end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(C_MEC=3.1*10**-12, M_TOC=190*10**3, R_TOC=160*10**6), start_f=2500, stop_f=3500, num=5000),
    "101" : end_to_end_get_ar_response_norm_velocity(RaviczMiddleEar(C_MEC=0.77*10**-12, M_TOC=610*10**3, R_TOC=310*10**6), start_f=2500, stop_f=3500, num=5000)
}
...
def spectral_angle_v_r_toc(ear_dict:dict, ravicz_angles:list, annotate=False):
    ...
    ravicz_angles = [89.93, 77.84, 78.36, 64.1, 64.29, 64.73, 64.8]
    ...
    plt.legend()
    plt.savefig("ravicz_model/final_report_plots_notation/spectral_angle_v_r_toc.pdf")
    plt.show()
...
#last minute manual hacking!!!! (useful for presentation as well)
ravicz_angles_manu = spectral_angle_v_tm_coverage_for_latex(tm_and_effusion_modified_range_for_spectral_angle_norm_u_manual)
print(ravicz_angles_manu)
spectral_angle_v_r_toc(ear_dict_rav_sgar, ravicz_angles=ravicz_angles_manu)
...
def illustrate_how_angle_is_calculated(effused_dict):
    ...
    ax.set(xlabel="Frequency (Hz)", ylabel=r"$|Z^s_{\mathrm{EC}}|\thinspace(\mathrm{kg} \mathrm{m}^{-2} \mathrm{s}^{-1})$")
    plt.legend()
    plt.savefig("ravicz_model/final_report_plots_notation/illustrate_sgar.pdf")
    plt.show()
...
select_eff_sgar_dict_extra_manu = {
    "0": tm_and_effusion_modified_range_for_spectral_angle_norm_u_manual["0"], 
    # "25": tm_and_effusion_modified_range_for_spectral_angle_norm_u_manual["25"], 
    "100": tm_and_effusion_modified_range_for_spectral_angle_norm_u_manual["100"],
    "101": tm_and_effusion_modified_range_for_spectral_angle_norm_u_manual["101"],
    }
illustrate_how_angle_is_calculated(select_eff_sgar_dict)
# illustrate_how_angle_is_calculated(select_eff_sgar_dict_extra_manu)
...
def illustrate_issues_with_angle_calculation(ar_resp):
    ...
    ax0.set(xlabel="Frequency (Hz)", ylabel=r"$|Z^s_{\mathrm{EC}}|\thinspace(\mathrm{kg} \mathrm{m}^{-2} \mathrm{s}^{-1})$")
    ...
    plt.savefig("ravicz_model/final_report_plots_notation/illustrate_issue_with_angle.pdf")
    plt.show()
...
def healthy_ear_changing_one_parameter_impedance(ear_dict:dict, ear_canal=False):
    ...
    impedance_type = "me" if not ear_canal else "ec"
    plt.savefig(f"ravicz_model/final_report_plots_notation/{impedance_type}_impedance_bode_parametric.pdf")
    plt.show()
...
#TODO figure out why this keeps breaking
...
def healthy_ear_changing_one_parameter_acoustic_reflectometry(effused_dict:dict, reflectivity=True):
    ...
    plt.savefig("ravicz_model/final_report_plots_notation/param_binary_output_latex.pdf")
    plt.show()
...
def compare_ravicz_impedances_for_tympanometry(ear_dict:dict, ear_canal=False):
    ...
    impedance_type = "me" if not ear_canal else "ec"
    plt.savefig(f"ravicz_model/final_report_plots_notation/{impedance_type}_impedance_bode_tymp.pdf")
    plt.show()
